...
// Helper to check if proxy headers should be trusted
...
function normalizeModel(provider, model) {
  ...
  if (p === 'nvidia' && ml.includes('kimi-k2.5')) return 'kimi-k2.5';
  ...
  if (p === 'minimax' && ml.includes('minimax-m2.7')) return 'MiniMax-M2.7';
  return m;
}

function estimateMsgCost(msg) {
  return require('./pricing').estimateMsgCost(msg);
}
...
function setSameSiteCORS(req, res) {
  ...
  // Only trust X-Forwarded-Proto when TRUST_PROXY is enabled
  const trustedProto = trustProxyHeaders() && req.headers['x-forwarded-proto'] === 'https';
  const proto = req.socket.encrypted || trustedProto ? 'https' : 'http';
...
function httpsEnforcement(req, res) {
  ...
  // Only trust X-Forwarded-Proto when TRUST_PROXY is enabled
  const trustedProto = trustProxyHeaders() && req.headers['x-forwarded-proto'] === 'https';
  if (req.socket.encrypted || trustedProto) return true;
