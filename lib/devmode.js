...
function setupHotReload(rootDir) {
  if (!IS_DEV) return;
  const watchFiles = ['index.html', 'styles.css', 'core-helpers.js', 'render-helpers.js', 'auth-ui.js', 'app.js', 'dashboard-ui.js', 'sessions-ui.js', 'misc-ui.js', 'system-ui.js', 'memory-ui.js', 'bootstrap.js'];
...
const API_DOCS = {
  ...
  endpoints: [
    ...
    // File management
    { method: 'GET',  path: '/api/csrf-token',        description: 'Get a CSRF token for mutations', auth: true },
    { method: 'GET',  path: '/api/memory',             description: 'Alias for /api/memory-files', auth: true },
    { method: 'GET',  path: '/api/memory-file',        description: 'Read a memory file (?path=)', auth: true, params: [{ name: 'path', type: 'string', description: 'File path (e.g., MEMORY.md, memory/file.md)' }] },
    { method: 'GET',  path: '/api/key-file',           description: 'Read a workspace file (?path=)', auth: true, params: [{ name: 'path', type: 'string', description: 'File name' }] },
    { method: 'POST', path: '/api/key-file',           description: 'Update a workspace file (JSON body: {path, content})', auth: true },
    // Cron management
    { method: 'POST', path: '/api/cron/:id/toggle',    description: 'Toggle a cron job enabled/disabled', auth: true },
    { method: 'POST', path: '/api/cron/:id/run',       description: 'Manually trigger a cron job', auth: true },
    // Actions
    ...
    { method: 'POST', path: '/api/action/restart-tailscale',  description: 'Restart Tailscale daemon', auth: true },
    { method: 'POST', path: '/api/action/restart-claude',     description: 'Restart Claude tmux session', auth: true },
    { method: 'POST', path: '/api/action/update-openclaw',    description: 'Update OpenClaw via npm', auth: true },
    { method: 'POST', path: '/api/action/check-update',       description: 'Check for OpenClaw updates', auth: true },
    { method: 'POST', path: '/api/action/clear-cache',        description: 'Clear all caches', auth: true },
    { method: 'POST', path: '/api/action/gc',                 description: 'Run git gc on all repos', auth: true },
    { method: 'POST', path: '/api/action/disk-cleanup',       description: 'Clean up disk space', auth: true },
    { method: 'POST', path: '/api/action/sys-update',         description: 'Run system package updates', auth: true },
    { method: 'POST', path: '/api/action/kill-tmux',          description: 'Kill Claude tmux session', auth: true },
  ]
};
